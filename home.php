    ...
    <div class="flex flex-row relative">
        ...
        <div class="m-10 w-full">
            ...
            <div class="flex justify-between bg-neutral-700  p-5 rounded-xl mb-5">
                ...
                <div class="grid grid-cols-5 grid-rows-5 gap-4 h-dvh text-black">
                    <div class="col-span-2 row-span-5 bg-neutral-300 rounded-md p-5">
                        <p class="font-bold text-xl mb-5">Announcement | Memo</p>
                        
                        <div class="overflow-y-auto overflow-x-hidden pr-2" style="height: 90dvh">
                            <?php
                            require 'controllers/db_conn.php';
                            ...
                            if ($posts_result->num_rows > 0) {
                                while ($post = $posts_result->fetch_assoc()) {
                                    echo '<div class="card bg-neutral-200 text-black mb-5">';
                                    ...
                                    echo '<p class="text-justify whitespace-pre-wrap">' . htmlspecialchars($post['content']) . '</p>';
                                    if (!empty($post['image_path'])) {
                                        echo '<img src="' . htmlspecialchars($post['image_path']) . '" alt="Post Image" class="w-96 h-96 rounded-md">';
                                    }
                                    echo '</div>';
                                    echo '</div>';
                                }
                            } else {
                                echo '<p class="text-center font-semibold">No posts available.</p>';
                            }
                            ?>
                        </div>
                    </div>

                    <div class="col-span-2 row-span-5 col-start-3 bg-neutral-300 rounded-md p-5 h-dvh">
                        <p class="font-bold text-xl mb-5">Events</p>
                    ...
                    <div class="row-span-1 col-start-5 bg-neutral-300 h-dvh rounded-md p-5">
                        <p class="font-bold text-xl mb-5">Stats</p>
                        <div class="grid grid-cols-1 gap-4 h-[90dvh]">
                            <div class="flex justify-center items-center">
                                <?php
                                require 'controllers/db_conn.php'; 
                                $sql = "SELECT COUNT(*) AS inmate_count FROM inmates WHERE status = 'detained'";
                                $result = $conn->query($sql);
                                if ($result->num_rows > 0) {
                                    $row = $result->fetch_assoc();
                                    echo "<div class='flex flex-col items-center'>";
                                    echo "<p class='text-5xl font-bold text-green-700 mb-5'>" . $row["inmate_count"] . "</p>";
                                    echo "<p class='text-4xl font-semibold'>Inmates</p>";
                                    echo "</div>";
                                }
                                ?>
                            </div>

                            <div class="divider before:border-t-2 before:border-black after:border-t-2 after:border-black"></div>
                            ...
                            <div class="flex justify-center items-center">
                                ...
                                if ($result->num_rows > 0) {
                                    $row = $result->fetch_assoc();
                                    echo "<div class='flex flex-col items-center'>";
                                    echo "<p class='text-5xl font-bold text-blue-700 mb-5'>" . $row["visitor_count"] . "</p>";
                                    echo "<p class='text-4xl font-semibold'>Visitors</p>";
                                    echo "</div>";
                                }
                                ?>
                            </div>
                            ...
                            <div class="flex justify-center items-center">
                                <?php
                                $sql = "SELECT COUNT(*) AS user_count FROM users WHERE status = 'Verified'";
                                $result = $conn->query($sql);
                                if ($result->num_rows > 0) {
                                    $row = $result->fetch_assoc();
                                    echo "<div class='flex flex-col items-center'>";
                                    echo "<p class='text-5xl font-bold text-purple-700 mb-5'>" . $row["user_count"] . "</p>";
                                    echo "<p class='text-4xl font-semibold'>Users</p>";
                                    echo "</div>";
                                }
                                $conn->close();
                                ?>
