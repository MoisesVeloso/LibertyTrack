...
<body class="bg-gray-100 flex justify-center items-center h-dvh">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        verifyButton.addEventListener('click', () => {
            retakeButton.style.display = 'none';
            verifyButton.innerHTML = '<span class="align-middle loading loading-spinner"></span>Verifying</button>';
                    ...
                    if (data.success) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                const formDataJson = sessionStorage.getItem('logFormData');
                                const formData = new FormData();
                                for (const [key, value] of Object.entries(JSON.parse(formDataJson))) {
                                    formData.append(key, value);
                                }
                                formData.append('inmate_id', inmateId);

                                fetch('controllers/addLog.php', {
                                    method: 'POST',
                                    body: formData
                                })
                                .then(response => response.json())
                                .then(data => {
                                    if (data.success) {
                                        Swal.fire('Saved!', 'Log added successfully.', 'success').then(() => {
                                            window.opener.postMessage('logAdded', '*');
                                            window.close();
                                        });
                                    } else {
