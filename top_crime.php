...
<body class="bg-neutral-200">
    ...
    <div class="flex flex-row relative">
        ...
        <div class="m-10 w-full">
            ...
            <div class="grid grid-cols-6 grid-rows-6 gap-4">
                <div class="col-span-6 row-span-6 bg-neutral-300 p-5 rounded-md text-black">
                    <div class="flex justify-between items-center mb-2">
                        <div class="flex flex-row gap-2">
                            <select id="monthSelect" class="select select-bordered bg-neutral-200">
                        ...
                        <button class="btn btn-accent" id="saveChartPdf">
                    ...
                    <div class="w-full" style="height: 700px;">
                        <canvas id="ageCrimeChart"></canvas>
                    </div>
                </div>
            </div>
        </div>
    ...
    function updateAgeCrimeChart() {
        const chartElement = document.getElementById('ageCrimeChart');
    ...
    document.getElementById('monthSelect').addEventListener('change', updateAgeCrimeChart);
    document.getElementById('yearSelect').addEventListener('change', updateAgeCrimeChart);

    updateAgeCrimeChart();

    $('#saveChartPdf').on('click', function() {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF('landscape');

        doc.setFontSize(12);
        doc.text("NATIONAL POLICE COMMISSION", 148.5, 15, { align: "center" });
        doc.text("PHILIPPINE NATIONAL POLICE CAPITAL REGION POLICE OFFICE", 148.5, 20, { align: "center" });
        doc.text("MANILA POLICE DISTRICT", 148.5, 25, { align: "center" });
        doc.text("MORIONES POLICE STATION (PS-2)", 148.5, 30, { align: "center" });
        doc.text("J. Nolasco corner Morga Sts. Tondo, Manila", 148.5, 35, { align: "center" });

        doc.addImage('img/pnp_logo.png', 'PNG', 10, 10, 20, 20);
        doc.addImage('img/police-logo.png', 'PNG', 260, 10, 20, 20);

        const canvas = document.getElementById('ageCrimeChart');
        const imgData = canvas.toDataURL('image/png');
        doc.addImage(imgData, 'PNG', 15, 40, 260, 160); 

        doc.save("top_crime_chart.pdf");
    });
    </script>
</body>
</html>